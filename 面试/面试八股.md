...
- `HashMap`独立了hash算法，通过`key`，`value`多次计算hashCode值，减少了重复性
- `HashMap`的遍历顺序是不确定的
- `HashMap`最多只允许一条记录的`key`为`null`，允许多条记录的`value`为`null`
- `HashMap`要求映射中的`key`是不可变对象，即该对象在创建后它的hashCode值不会被改变

> `Hashtable`继承的是`Dictionary`抽象类，实现了`Map`、`Cloneable`和`Serializable`接口
>
> - `Hashtable`是线程安全的，任一时间只有一个线程能写`Hashtable`
> - `Hashtable`直接使用`key`的`hashcode`值
>
> `LinkedHashMap`：`HashMap`的一个子类，保存了记录的插入顺序，在用`Iterator`遍历`LinkedHashMap`时，先得到的记录肯定是先插入的
...
1. 底层数组是`Node[]`（`Node<K,V>[] table`）
   1. `Node`是`HashMap`的一个内部类，实现了`Map.Entry`接口，本质是就是一个映射（键值对）
...
- `threshold`：HashMap扩容的临界值（**容量*负载因子**：16*0.75 = 12），即所能容纳的最大的`Node`（键值对）个数
...
static final int hash(Object key) {
    int h;
    // 取hashCode值，然后高位参与运算，最后取模
    return (key == null) ? 0 : (h = key.hashCode()) ^ (h >>> 16);
}
```

先判断`Node`数组`table[]`是否为空或为`null`，否则执行`resize()`进行扩容

然后调用`key`所在类的`hascode()`方法计算出当前`key`的hashcode值，通过**散列函数**找到`key`在`Node`数组中的插入位置
...
> `modCount`：主要用来记录`HashMap`**内部结构发生变化的次数**，主要用于迭代的快速失败，内部结构发生变化指的是结构发生变化，例如`put`新键值对，但是某个`key`对应的`value`值被覆盖不属于结构变化
...
final V putVal(int hash, K key, V value, boolean onlyIfAbsent,
    ...
    if ((tab = table) == null || (n = tab.length) == 0)
        // 如果是首次put（为空），使用resize()创建一个Node<K,V>数组
        n = (tab = resize()).length;
    // 根据key的hashcode值找到要存储的位置index，查看是否有数据
...
- **扩容之后，要重新计算对象的位置，然后把它们移动到合适的位置上去**
  - 在旧数组中同一条Node链上的元素，通过重新计算索引位置后，有可能被放到了新数组的不同位置上
...
final Node<K,V>[] resize() {
    ...
    int newCap, newThr = 0;
    // table数组是有长度的，即不是第一次put，不用创建，直接进行扩容处理
    if (oldCap > 0) {
        // 超过最大值就不再扩充了，就只能碰撞了
