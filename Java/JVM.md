...
主流的垃圾收集器都是**依靠可达性分析算法来判断对象是否存活**，而可达性分析算法要求全过程都基于一个能**保证一致性的快照**中才能进行分析，这要求停顿所有的用户线程
...
- **黑色**：表示对象已经被垃圾收集器访问过，且**这个对象的所有引用都已经被扫描过**
  - 在可达性分析结束的阶段吗，黑色的对象表示是安全存活的，**如果有其他对象引用指向了黑色对象，则无需重新扫描一遍**
...
2. **初始标记阶段**：GC Roots将直接关联的对象标记为灰色
3. **并发标记阶段**：扫描整个引用链
   1. 没有子节点的话，将本节点变为黑色
