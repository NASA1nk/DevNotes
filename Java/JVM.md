...
**复制算法多用于新生代，因为新生代的对象98%都要被回收，所以也不需要划分成2块**

- 将新生代划分为一块较大的eden空间和两块较小的survivor空间
...
- **收集器在这一步必须暂停用户线程**
- 必须在一个保证一致性的快照中进行根节点枚举
  - **整个枚举期间根节点集合的对象引用关系不能变化**
- JVM可以直接知道哪些地方有对象引用
  - 使用Oop数据结构存储，直接扫描即可
...
需要在垃圾收集时让所有线程到达最近的安全点，然后停顿下来（Stop The World）

- **抢占式中断**
  - 垃圾收集时，系统将所有用户线程中断
  - 如果发现有线程不在安全点，就恢复执行，知道跑到安全点
- **主动式中断**
  - **不对线程进行操作**
...
- 用户线程执行到安全区域的代码时会标识自己进入了安全区域，JVM在这段时间发起垃圾收集就不用管这些在安全区域的线程
...
主流的垃圾收集器都是依靠可达性分析算法来判断对象是否存活，而可达性分析算法要求全过程都基于一个能保证一致性的快照中才能进行分析，这要求停顿所有的用户线程
...
- **白色**：表示对象尚未被垃圾收集器访问过
  ...
  - 在可达性分析结束的阶段，白色的对象表示不可达
- **黑色**：表示对象已经被垃圾收集器访问过，且**这个对象的所有引用都已经被扫描过**
  - 黑色的对象表示已经访问过，是安全存活的，**如果有其他对象引用指向了黑色对象，则无需重新扫描一遍**
...
- 如果将原本消亡的对象标记为存活，是可以容忍的，只是产生了一点逃过本次垃圾收集的浮动垃圾，下次收集即可
- 如果**把原本存活的对象标记为已消亡**，则会产生致命的后果，程序肯定会因此发生错误
  - 即**黑色对象被标记为白色**

> 对象只有被黑色对象引用才能存活，扫描完成后，黑色的对象就是存活对象，白色的对象就消亡可回收的对象
>
> 黑色对象不会再被扫描，所以修改引用关系后可能会出现危险

#### 解决并发问题

当且仅当以下两个条件同时满足，才会出现**对象消失**（把原本存活的对象标记为已消亡）问题

- 赋值器插入了一条或多条从黑色对象到白色对象的新引用
- 赋值器删除了全部从灰色对象到该白色对象的直接或间接引用

所以要解决并发扫描时的对象消失问题，**只要破坏其中任意一个条件即可**，所有有两种解决方法
