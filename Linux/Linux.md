...
- 用户在命令行中输入命令，运行在后台的shell把命令转换为指令代码发送给OS
...
基于Unix或类Unix操作系统一般情况下都将bash作为**默认的终端shell**
