...
- 子类的无参构造器中默认调用父类的**无参**构造（隐式），当显式调用时，父类的构造器必须在最前面
- 当父类存在有参构造时，父类无参构造也必须是显式。如果此时父类没有显示的定义无参构造（即没有无参构造），则**子类的无参构造中也必须显示调用父类的有参构造**，而不能隐式调用父类的无参构造。

- 子类拥有父类的所有`public`修饰的属性和方法（`private`的属性和方法不会被继承）
...
``super`表示父类的`public`修饰的属性和方法（`private`的属性和方法不会继承，所以`super`也无法调用）
