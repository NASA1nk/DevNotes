...
此时如果来读刚刚插入的数据，且读语句发生在更新语句后不久，**内存中的数据都还在**
...
- 假设要删掉某一个记录，**InnoDB引擎只会把这个记录标记为删除**，因为如果之后要再插入相同（位置）记录时，可能会复用这个位置，**所以磁盘文件的大小并不会缩小**
- 假设要删掉了一个数据页上的所有记录，那么整个数据页都可以复用了，**此时就没有相同位置的要求了**
- 假设要把整个表的数据，那么所有的数据页都会被标记为可复用，**但是磁盘上文件不会变小**

**所以通过`delete`命令是不能回收表空间的**

**插入数据**

- 如果数据是按照索引递增顺序插入的，那么索引是紧凑的，**但如果数据是随机插入的，就可能造成索引的数据页分裂**，**页分裂也会造成有未使用的空间**

**更新索引**

- 可以理解为删除一个旧的值，再插入一个新值，也有同样的问题

## 重建表

`alter table A engine=InnoDB`

- **重建表就可以回收这些未使用的空间，达到收缩空间的效果**

- MySQL5.6版本引入Online DDL，可以在对表操作的同时重建表，**只要将复制表数据过程中对表的操作记录在日志中，在复制完成后，对新表做操作即可**

> 重建会扫描原表数据和构建临时文件，对于很大的表来说这个操作是很消耗IO和CPU资源的
>
> 在重建表的时候，InnoDB引擎不会把整张表占满，每个页留了1/16给后续的更新用，也就是说，其实重建表之后不是最紧凑的

### Inplace和copy

根据旧表重建出来的数据是放在`tmp_file`里的，这个临时文件是InnoDB引擎在内部创建出来的，整个DDL过程都在InnoDB引擎内部完成

- 对于server层来说，没有把数据挪动到临时表，是一个**原地操作**，即inplace

- DDL过程如果是Online的，就一定是inplace的

> 所以有一个1TB的表，磁盘空间是1.2TB，是不能做一个inplace的DDL的，因为`tmp_file`也是要占用临时空间的

与inplace在InnoDB引擎层创建临时文件不同，**copy则是在server层建临时表**

- 两者都是需要临时空间的，只是inplace在server层不可见

### MDL锁

`alter`命令属于DDL，**DDL之前是要拿MDL表锁的**，之所以可以直接复制是**因为这个MDL写锁在真正拷贝数据之前就退化成读锁了**，MDL读锁不会阻塞增删改操作

1. 一开始拿MDL写锁，是为了防止其他操作修改表结构，直到记录的日志建立好
2. 此时MDL写锁就可以退化成MDL读锁，**MDL读锁可以允许其他线程DML操作，并且阻塞其他线程DDL操作**
   1. DML语句加的是MDL读锁

最后把数据复制到临时表的时候还会获取一次MDL写锁，防止其他DML操作

> MDL写锁持有时间比较短，所以可以称为Online
...
`count()`是一个聚合函数，对于返回的结果集，一行行地判断，如果`count()`的参数不是`NULL`累计值就加 1，否则不加，最后返回累计值

在不同的MySQL引擎中，`count(*)`有不同的实现方式

- MyISAM引擎把一个表的总行数存在了磁盘上，因此执行`count(*) `的时候会直接返回，效率很高
- InnoDB 引擎在执行`count(*)`的时候，需要把数据一行一行地从引擎里面读出来，然后累积计数

> 讨论的是没有过滤条件的`count(*)`，如果加了`where`条件的话，MyISAM引擎也是不能返回得这么快的

由于多版本并发控制（MVCC）的原因，InnoDB引擎的表**应该返回多少行也是不确定的**

- **默认隔离级别是RR，但是RR的查询是快照读**，所有有些行不一定可见

> 而MyIASM引擎不支持事务，也不支持MVCC，所有线程看到的记录都是一样的，所以可以直接记录一个总数

**查询优化**

- 因为InnoDB引擎表的**主键索引树的叶子节点是数据**，普通索引树的叶子节点是主键值，**所以普通索引树比主键索引树小很多**
- 所以对于`count(*)`这样的**遍历哪个索引树得到的结果逻辑上都是一样**的操作，**MySQL优化器会找到最小的那棵树来遍历**

> 在保证逻辑正确的前提下，尽量减少扫描的数据量，是数据库系统设计的通用法则之一
>
> `show table status`显示的行数是采样估算的， 不准

### 如何在保证性能的情况下保存计数

#### 缓存

使用Redis保存行数，并在插入/删除时候+1/-1

**问题**

- Redis的数据不能永久地留在内存里，需要定期地持久化存储起来，但仍然可能丢失更新
- 如果在数据表中插入了一行，Redis中保存的值也+1，然后Redis异常重启了，重启后要从存储Redis数据的地方把这个值读回来，而刚刚+1的这个计数操作却丢失了

> 可以在Redis异常重启以后，到数据库里面单独执行`count(*)`获取真实的行数，再把这个值写回到Redis 里

将计数保存在缓存系统中的方式，还不只是丢失更新的问题，即使Redis正常工作，**这个值还是逻辑上不精确的**

- 主要原因是因为**MySQL插入一行数据**跟**Redis计数+1**这两个操作是分开的，不是原子性的，所以就可能在中间过程因为某些并发问题
- 比如两个不同的事务，一个在MySQL中插入数据，一个在Redis中读取行数，就会出现不一致的问题
...
- 可以解决缓存的崩溃丢失问题
- 计数+1和插入数据操作是在一个事务内的，逻辑一致（对外部事务来说）

**那么在这个事务序列里，应该先插入操作记录，还是应该先更新计数表呢？**

先插入新纪录

- **因为插入新记录只会影响到行锁和间隙锁**，而更新计数表会占用计数表的写锁，而很多其他事务的插入操作就必须阻塞等待

### `count(*)`、`count(主键 id)`、`count(字段)` 和`count(1)`等的性能差别

- `count(*)`、`count(主键 id)`和`count(1)`都表示**返回满足条件的结果集的总行数**

- `count(字段)`表示**返回满足条件的数据行里面，参数"字段"不为`NULL`的总个数**，即是单独判断该字段
- 按照效率排序：`count(字段)` < `count(主键 id)` < `count(1)` ≈ `count(*)`，尽量使用`count(*)`

#### `count(主键 id)`

InnoDB引擎会遍历整张表，**把每一行的id值都取出来返回给server层**，server 层拿到id后，判断是不可能为空的，按行累加

#### `count(1)`

InnoDB引擎会遍历整张表，**但不取值，server层对于返回的每一行，放一个数字1进去**，判断是不可能为空的，按行累加

> 单看两个用法的差别，`count(1)`执行得要比`count(主键 id) `快，因为从引擎返回id会涉及到解析数据行，以及拷贝字段值的操作

#### `count(字段)`

- 如果这个字段是定义是`not null`，一行行地从记录里面读出这个字段，判断不能为`null`，按行累加
- 如果这个字段定义是允许为`null`，那么执行的时候，**判断到有可能是`null`，还要把值取出来再判断一下，不是`null`才累加**
